...
import { Exception, NotImplementedError } from "@jsext/error";
import { trim } from "@jsext/string";
import { readAsArray } from "@jsext/reader";
import { isDeno } from "@jsext/env";
...
describe("jsext.parallel", () => {
    ...
    it("omit suffix", async function () {
        ...
        // @ts-ignore because allowJs is not turned on
        const { default: sum } = jsext.parallel(() => import("../examples/sum.js"));
        strictEqual(await sum(1, 2, 3, 4, 5, 6, 7, 8, 9), 45);

        if (typeof Bun === "object") {
            const { default: sum } = jsext.parallel(() => import("../examples/sum2.ts"));
            strictEqual(await sum(1, 2, 3, 4, 5, 6, 7, 8, 9), 45);
        }
    });
