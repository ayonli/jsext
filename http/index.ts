 ...
 * Deno:
 * ```sh
 * deno run --allow-net --allow-read jsr:@jsext/http [--port PORT] [DIR]
 * deno run --allow-net --allow-read jsr:@jsext/http <entry.ts>
 * ```
 * 
 * Bun:
 * ```sh
 * bun run node_modules/@jsext/http.ts [DIR] [--port PORT] [--parallel [NUM]]
 * bun run node_modules/@jsext/http.ts <entry.ts> [--parallel [NUM]]
 * ```
 * 
 * Node.js:
 * 
 * ```sh
 * tsx node_modules/@jsext/http.ts [DIR] [--port PORT] [--parallel [NUM]]
 * tsx --import=@jsext/http <entry.ts> [--parallel [NUM]]
 * # or
 * node node_modules/@jsext/esm/http.js [DIR] [--port PORT] [--parallel [NUM]]
 * node -r @jsext/http <entry.js> [--parallel [NUM]]
 * ```
 * @module
 */
...
import { Range, ifMatch, ifNoneMatch, parseRange } from "./util.ts";
...
if ((isDeno || isBun || isNode) && isMain(import.meta) && isThisMain(import.meta)) {
    startServer(args);
} else if (isNode && process.execArgv.some(arg => arg.endsWith("@jsext/http"))) {
    ...
    if (args.length && (
        (options["require"] as string[] | undefined)?.includes("@jsext/http") ||
        (options["import"] as string[] | undefined)?.includes("@jsext/http")
    )) {
        startServer(args);
    }
}
